...
export async function fetchUserFromToken(socket: Socket, userRepo: IUserRepository) {
...
export async function getCreatorName(creator: string, socket: Socket, userRepo: IUserRepository) {
    socket.use(async (_, next) => {
        const token = socket.handshake.auth.token as string;
        if (!token) next(new Error(errorMessage.get(errors.TOKEN_NOT_FOUND)))
